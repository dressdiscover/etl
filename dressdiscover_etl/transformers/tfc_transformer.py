...
class TfcTransformer(_Transformer):
    ...
    def __init__(self):
        ...
        agent_qualifiers_etree.parse(
            os.path.join(os.path.dirname(__file__), "agent-qualifiers.xml")
        )
        ...
        for property_etree in agent_qualifiers_etree.iter():
            if property_etree.tag != RDF_NS + "Property":
                continue
            agent_qualifier_code = property_etree.attrib[RDF_NS + "about"].rsplit(
                "#", 2
            )[-1]
        ...
        if qualifier in ("ARK", "LOCAL-CONT-NO", "OTHER"):
            ...
            high_res_image = Image.create(
                ...
                uri=URIRef(
                    ...
                    + record_identifier[len("info:ark") :]
                    + "/m1/1/high_res/"
                ),
            )
            ...
            self._logger.warn(
                "ignoring unknown identifier qualifier '{}' on record {}",
                qualifier,
                record_identifier,
            )
    ...
    def _parse_record_metadata_meta_element(self, element, record_identifier, **kwds):
        ...
        if qualifier is None:
            self._logger.warning(
                "meta element without qualifier on record {}", record_identifier
            )
            return
        ...
        if qualifier in (
            ...
            self._logger.warning(
                "ignoring unknown meta qualifier '{}' on record {}",
                qualifier,
                record_identifier,
            )
    ...
    def _parse_record_metadata_rights_element(
        ...
        if qualifier is None:
            self._logger.warning(
                "rights element without qualifier on record %s", record_identifier
            )
            return
    ...
    def transform(self, record_etrees: Tuple[ElementTree, ...]) -> Graph:
        ...
        for record_etree_i, record_etree in enumerate(record_etrees):
            ...
            metadata_etree = record_etree.find("metadata").find(
                self._UNTL_NS + "metadata"
            )
            ...
            for etree in metadata_etree:
                ...
                try:
                    ...
                    self._logger.warning(
                        "no such method '%s' for record %s with tex" "t '%s'",
                        method_name,
                        record_identifier,
                        etree.text.strip(),
                    )
